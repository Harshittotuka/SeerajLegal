...
public function update(Request $request, $id)
{
    try {
        ...
        $newFirst = isset($data['firstName'])
            ? preg_replace('/\s+/', '', strtolower($data['firstName']))
            : preg_replace('/\s+/', '', strtolower($intern->firstName));

        $newLast = isset($data['lastName'])
            ? preg_replace('/\s+/', '', strtolower($data['lastName']))
            : preg_replace('/\s+/', '', strtolower($intern->lastName));
        ...
        // 4) Else if name changed but no new file → rename existing file
            ...
            ($request->filled('firstName') || $request->filled('lastName'))
            && $intern->resumePath
            && file_exists(public_path($intern->resumePath))
        ) {
