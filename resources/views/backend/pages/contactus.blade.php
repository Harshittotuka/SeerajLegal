...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    form.querySelectorAll('input, textarea').forEach(input => {
        ...
        // Ensure URLs are valid by adding "https://" if missing
        if (input.id.includes('_link') || input.id === 'website_link') {
            if (value && !value.match(/^https?:\/\//)) {
                value = 'https://' + value; // Prepend "https://" if missing
            }
        }
    ...
    console.log("Attempting to update details...", updatedDetails); // Debugging log

    try {
        const response = await fetch('http://127.0.0.1:8000/api/update-personal-details', {
            method: 'PUT',
